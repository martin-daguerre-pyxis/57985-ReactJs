...
const Article = ({ children, picture, title, description, link, className }) => {
    return (
        <div className={className + "container w-full mb-8 p-12 mx-auto lg:px-32 lg:mb-16 text-xl font-medium text-center text-purple-800 rounded-b-full bg-purple-50"}>
            <div className={className + "flex flex-col gap-3 lg:flex-row text-center sm:text-left"}>
                <div className="flex-grow">
                    <div style={{ fontSize: 'clamp(2.25rem, 1.4224rem + 4.1379vw, 3.75rem)' }}
                        className='font-title'>{title ? title : 'Articulo sin título'}</div>
